...
class MultiTrackPianoRollDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, 
        ...
        self.tracks = tracks
        self.directories = os.listdir(self.root_dir)[:n_files]
...
def track_to_tok(tracks):
    print("tracks")
    print(tracks)
    tok = [np.expand_dims(np.argmax(t, axis=1) * 128 + np.max(t, axis=1), axis=1) for t in tracks]
    print("tok")
    print(tok)
    return tok
...
def test_transform(tracks, instrument=4):
    sample = tracks[instrument].flatten().tolist()
    return sample, [START_TOKEN] + sample + [END_TOKEN]

def gen_test_transform(tracks, instrument=4):
    sample = tracks[instrument].flatten().tolist()
    return sample
    

transform = lambda x: test_transform(pad_tracks(track_to_tok(x)))
gen_transform = lambda x: gen_test_transform(pad_tracks(track_to_tok(x), pad_to=160))
